...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-gray-200 p-4">
        ...
        <div class="lg:w-2/4 mx-auto py-8 px-6 bg-white rounded-xl">
            <h1 class="font-bold text-5xl text-center mb-8">Control de Pacientes</h1>

            <div class="mb-6">
                <form class="flex flex-col space-y-4" action="/patientcr" method="POST" >
                    @csrf
                    <input type="text" name="name" placeholder="Name" class="py-3 px-4 bg-gray-100 rounded-xl">
                    <input type="text" name="lastname" placeholder="Lastname" class="py-3 px-4 bg-gray-100 rounded-xl">
                    <input type="text" name="email" placeholder="email" class="py-3 px-4 bg-gray-100 rounded-xl">
                    <input type="date" id="date" name="birth" min="1900-01-01"/>
                    <input type="text" name="phone" placeholder="Phone numer" class="py-3 px-4 bg-gray-100 rounded-xl">
                ...
                <div class="mt-2">
                    @foreach($patients as $patient)
                        <div 
                            ...
                            $patient->isDone ? 'bg-green-200':''])
                        >
                        @php
                            $fechaNacimiento = new DateTime($patient->birth);
                            $hoy = new DateTime();
                            $edad = $hoy->diff($fechaNacimiento);
                        @endphp
                            <div class="flex-1 pr-8">
                                <h3 class="text-lg font-semibold" id="data">{{$patient->lastname}}, {{ $patient->name }}. Edad: {{$edad->y}} años</h3>
                            </div>
                            <div class="flex space-x-3">
                                <form method="post" action="/ptpre/{{ $patient->id }}">
                                    @csrf    
                                    @method('PATCH')
                                ...
                                <form method="post" action="/ptdel/{{ $patient->id }}">
                                    @csrf    
                                    @method('DELETE')
