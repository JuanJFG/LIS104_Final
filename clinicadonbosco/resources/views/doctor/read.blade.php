...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-gray-200 p-4">
        ...
        <div class="lg:w-2/4 mx-auto py-8 px-6 bg-white rounded-xl">
            <h1 class="font-bold text-5xl text-center mb-8">Control de Doctores</h1>

            <div class="mb-6">
                <form class="flex flex-col space-y-4" action="/doctorcr" method="POST" >
                    @csrf
                    <input type="text" name="name" placeholder="Nombre" class="py-3 px-4 bg-gray-100 rounded-xl">
                    <input type="text" name="lastname" placeholder="Apellido" class="py-3 px-4 bg-gray-100 rounded-xl">
                    <input type="text" name="email" placeholder="Correo electrónico" class="py-3 px-4 bg-gray-100 rounded-xl">
                    <input type="date" id="date" name="birth" min="1900-01-01"/>
                    <input type="number" name="phone" placeholder="Número de teléfono" class="py-3 px-4 bg-gray-100 rounded-xl">
                    ...
                    <input type="number" name="license" placeholder="Licencia médica" class="py-3 px-4 bg-gray-100 rounded-xl">
                ...
                <div class="mt-2">
                    @foreach($doctors as $doctor)
                        <div 
                            ...
                            $doctor->isDone ? 'bg-green-200':''])
                        >
                        @php
                            $fechaNacimiento = new DateTime($doctor->birth);
                            $hoy = new DateTime();
                            $edad = $hoy->diff($fechaNacimiento);
                        @endphp
                            <div class="flex-1 pr-8">
                                <h3 class="text-lg font-semibold" id="data">{{$doctor->lastname}}, {{ $doctor->name }}. Edad: {{$edad->y}} años.   {{$doctor->speciality}}</h3>
                            </div>
                            <div class="flex space-x-3">
                                <form method="post" action="/dcpre/{{ $doctor->id }}">
                                    @csrf    
                                    @method('PATCH')
                                ...
                                <form method="post" action="/dcdel/{{ $doctor->id }}">
                                    @csrf    
                                    @method('DELETE')
